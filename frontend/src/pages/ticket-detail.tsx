    ...
    const handleReserve = async (e: React.FormEvent) => {
        ...
        try {
            const response = await ticketApi.reserve({
                ...
                seat_info: reserveData.seat_info,
            });
                        ...
                        <div>
                            ...
                            {ticket.remaining_capacity > 0 ? (
                                <form onSubmit={handleReserve} className="space-y-4">
                                    ...
                                    <div>
                                        <label className="block text-gray-700 text-sm font-bold mb-2">شماره جایگاه یا صندلی (اختیاری)</label>
                                        <input
                                            type="text"
                                            value={reserveData.seat_info}
                                            onChange={(e) => setReserveData({ ...reserveData, seat_info: e.target.value })}
                                            placeholder="مثلا: ردیف ۲ صندلی ۱۰"
                                            className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary transition-all bg-white text-gray-900"
                                            disabled={isReserving}
                                        />
                                    </div>
