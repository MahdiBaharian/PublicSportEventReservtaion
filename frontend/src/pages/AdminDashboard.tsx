...
export default function AdminDashboard() {
  const [activeTab, setActiveTab] = useState<'reports' | 'reservations' | 'users' | 'addTicket'>('reports');
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (activeTab === 'reports') {
        ...
        res = await adminApi.getReservations();
      ...
      if (res && !res.error) {
        ...
        setError(res?.error || 'خطا در دریافت اطلاعات');
      }
    } catch (err) {
  ...
  const handleCancelReservation = async (id: number) => {
    try {
      await adminApi.updateReservationStatus(id, 'cancelled');
      fetchData();
    } catch (err) {
  ...
  const handleCreateTicket = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.error) {
        ...
        setMessage('بلیت با موفقیت ایجاد شد.');
        setTicketData({
          ...
          <ul className="space-y-2">
            ...
            <li>
              <button
                ...
                className={`w-full text-right px-4 py-3 rounded-lg font-bold transition-colors ${activeTab === 'reservations' ? 'bg-blue-600 text-white' : 'hover:bg-gray-100 text-gray-700'}`}
              >
                مدیریت رزروها
              </button>
            </li>
          ...
          {loading && activeTab !== 'addTicket' ? (
            ...
            <>
              {activeTab === 'reports' && (
                <div>
                  ...
                  <div className="space-y-4">
                    {data.length === 0 ? <p className="text-gray-500 text-center py-10">هیچ گزارشی یافت نشد.</p> : data.map((item: any, index: number) => (
              ...
              {activeTab === 'reservations' && (
                <div>
                  <h2 className="text-xl font-bold mb-6 border-b border-gray-100 pb-3">مدیریت رزروها</h2>
                    ...
                    <table className="w-full text-right border-collapse border border-gray-200 rounded-lg overflow-hidden">
                      <thead className="bg-gray-100">
                        <tr>
                          ...
                          <th className="p-4 font-bold text-sm text-gray-700 border-b">وضعیت</th>
                          <th className="p-4 font-bold text-sm text-gray-700 border-b">عملیات</th>
                        </tr>
                      </thead>
                      <tbody>
                        {data.length === 0 ? (
                          ...
                          <tr key={`res-${item.reservation_id}-${index}`} className="hover:bg-gray-50 border-b border-gray-100 transition-colors">
                            <td className="p-4 font-mono font-bold text-gray-900">{item.reservation_id}</td>
                            <td className="p-4 text-gray-600">{item.user_id}</td>
                            <td className="p-4 text-gray-600">{item.reservation_status}</td>
                            <td className="p-4 text-gray-600">
                              {item.reservation_status !== 'cancelled' && (
                                <button
                                  onClick={() => handleCancelReservation(item.reservation_id)}
                ...
                <div>
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-right border-collapse border border-gray-200 rounded-lg overflow-hidden">
                      ...
                      <tbody>
                        {data.length === 0 ? (
                          <tr><td colSpan={3} className="p-8 text-center text-gray-500 border-b">کاربری یافت نشد.</td></tr>
                  ...
                  <form onSubmit={handleCreateTicket} className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    ...
                    <div className="col-span-1 sm:col-span-2 mt-6 pt-4 border-t border-gray-100">
                      <button type="submit" disabled={loading} className="bg-gray-900 text-white font-bold py-3 px-8 rounded-lg hover:bg-black transition-colors w-full sm:w-auto shadow-md">
                        {loading ? 'در حال ثبت...' : 'ثبت نهایی بلیت در سیستم'}
                      </button>
                    </div>
                  </form>
                </div>
